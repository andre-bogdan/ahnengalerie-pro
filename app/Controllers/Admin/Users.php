...
class Users extends BaseController
{
     ...
     */
    /**
     * User speichern (MIT Email-Versand)
     */
    public function store()
    {
        ...
        $rules = [
            ...
            'send_email' => 'permit_empty|in_list[1]'  // NEU
        ];
        ...
        // Klartext-Passwort für Email zwischenspeichern
        $plaintextPassword = $this->request->getPost('password');
        $sendEmail = (bool) $this->request->getPost('send_email');

        // User anlegen
        $data = [
            'username' => $this->request->getPost('username'),
            'email' => $this->request->getPost('email'),
            'password' => $plaintextPassword,  // wird im UserModel gehasht
            'is_admin' => $this->request->getPost('is_admin') ? 1 : 0
        ];

        if ($this->userModel->insert($data)) {

            // NEU: Email versenden (wenn gewünscht)
            if ($sendEmail) {
                try {
                    $emailService = new \App\Libraries\EmailService();

                    $emailSent = $emailService->sendNewUserCredentials(
                        $data['email'],
                        $data['username'],
                        $plaintextPassword
                    );
                    ...
                    if ($emailSent) {
                        return redirect()->to('/admin/users')
                            ->with('success', 'Benutzer erfolgreich angelegt und Zugangsdaten per Email versendet!');
                    } else {
                        return redirect()->to('/admin/users')
                            ->with('warning', 'Benutzer erfolgreich angelegt, aber Email-Versand fehlgeschlagen. Prüfen Sie die Logs.');
                    }
                } catch (\Exception $e) {
                    log_message('error', 'Email-Versand fehlgeschlagen: ' . $e->getMessage());
                    return redirect()->to('/admin/users')
                        ->with('warning', 'Benutzer erfolgreich angelegt, aber Email-Versand fehlgeschlagen: ' . $e->getMessage());
                }
            }

            // Kein Email-Versand gewünscht
